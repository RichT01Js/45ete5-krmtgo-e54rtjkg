...
const listener = app.listen(process.env.PORT, function() {
  console.log("Your app is listening onn port " + listener.address().port);
});
...
client.ekoayarlar = {
  ...
  parabirimi: "<:parabirimi:1188541316607524865>", // dominic degistirme
  kazanmabirimi: "<a:slotwin2:1188541128383926312>",
  kaybetmebirimi: "<:slotlost:1188543753644933120>",
  store: "<a:store:1188549296413495346>",
  yapimcilar:"Richard, Dominic",
  botunuzunprefixi: ".", // dominic degistirme
  botunuzunidsi: "1001185378310565948", // dominic degistirme
  botismi: "SlotRed", // dominic degistirme
  renk: "RED", // dominic degistirme
  isimsiz: "Bilinmiyor",
  rastgelepara: false,
  minpara: false,
  maxpara: false,
  günlükpara: 5000000, // dominic degistirme
  dbloy: false,
  dblkey: false,
  dblmsj: false,
...
fs.readdir('./komutlar/', (err, files) => {
  ...
  files.forEach(f => {
    ...
    kurulum(`[SlotSystem] | ${pingKodları.help.name}`);
  ...
  });kurulum(`${files.length} Komut başarıyla kuruldu.`)
});
...
client.on('message', (message) => {
  ...
  if (message.mentions.has(client.user) && message.mentions.users.size === 1 && !message.content.includes('@everyone')) {
    const greetingEmbed = new Discord.MessageEmbed()
      ...
      .setTitle(client.ekoayarlar.botismi + ' Economy')
