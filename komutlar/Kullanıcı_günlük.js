...
exports.run = async (client, message, args) => {
  ...
  if (daily !== null && timeout - (Date.now() - daily) > 0) {
    ...
    return message.channel.send(`tekrar **.günlük** komutunu kullanabilmek için ${user}, ${time.hours}s ${time.minutes}d ${time.seconds} saniye beklemelisin <:deny:1188812328070819900>`);
  }

  db.set(`günlükkullanımgodareçdare-${user.id}`, Date.now());
  db.add(`bakiyeasreaper-${user.id}`, client.ekoayarlar.günlükpara);
  return message.channel.send(`${user} **__${client.ekoayarlar.günlükpara.toLocaleString()}${client.ekoayarlar.parabirimi}__** tutarında günlük bakiye aldınız! <:slotwin2:1188541128383926312>`);
};
