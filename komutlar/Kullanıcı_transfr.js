...
exports.run = async (client, message, args) => {
  ...
  if (transkllanç == kllanç) return message.channel.send(`${message.author}, Kendine para gönderemezsin <:deny:1188812328070819900>`);
  if (transkllanç.bot == true) return message.channel.send(`${message.author}, Botlara para gönderemezsin <:deny:1188812328070819900>`);
  if (!transkllanç) return message.channel.send(exports.help.usage);
  ...
  if (!hesapdurumu) {
    ...
    if (hesapdurumu) {
      if (!hesapismi) {
        ...
        if (hesapdurumu) {
          if (hesapismi) {
            if (bakiye < para) return message.channel.send(`${message.author},Para gönderebilmek için yeterli bakiyen yok. <:deny:1188812328070819900>`);
            if (!transhesapdurumu) return message.channel.send(`${message.author}, Para göndermek istediğin kullanıcının hesabı yok. <:deny:1188812328070819900>`);
            if (transhesapdurumu) {
              ...
              message.channel.send(`<:cuzdan:1188540963153530930> ${transkllanç} adlı kullanıcının hesabına **${formatCurrency(para, client.ekoayarlar.parabirimi)}** miktarında para yolladın. Açıklama: ${aciklama} `);
              
              transkllanç.send(client.ekoayarlar.botismi + ` \n<:cuzdan:1188540963153530930> ${message.author.tag} adlı kullanıcı **${guildName}** adlı sunucuda hesabınıza **${formatCurrency(para, client.ekoayarlar.parabirimi)}** miktarında para yolladı. Açıklama: **${aciklama}**`);
            }
          }
        }
      }
