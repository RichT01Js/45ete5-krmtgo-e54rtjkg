...
if (message.channel.type === 'dm') {
  ...
  const slotIcons = [
    ...
    { symbol: '💸', multiplier: 3 },
    { symbol: '💷', multiplier: 3 },
    { symbol: '<a:slotrushcoin:1188515137716506736>', multiplier: 3 },
    { symbol: '💳', multiplier: 5 },
  ];
  ...
  const spinningMessage = await message.channel.send(`Yükleniyor..`);
  ...
  setTimeout(async () => {
    for (let i = 0; i < totalIntervals; i++) {
      spinningMessage.edit(`<a:slotrushtrun:1188536403978616842> | ${newSymbolsArray[i]} | <a:slotrushtrun:1188536403978616842>`);
    ...
    if (winAmount > 0) {
      ...
      resultMessage += `${user} **${formatCurrency(betAmount)}${client.ekoayarlar.parabirimi}** tutarında bahis yatırdın ve **${formatCurrency(winAmount)}${client.ekoayarlar.parabirimi}** kazandın. ${client.ekoayarlar.kazanmabirimi}`;
      ...
      resultMessage += `${user} **${formatCurrency(betAmount)}${client.ekoayarlar.parabirimi}** tutarında bakiye yatırdın ve kaybettin... ${client.ekoayarlar.kaybetmebirimi}`;
    }
