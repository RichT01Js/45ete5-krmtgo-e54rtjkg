...
const achievementCategories = {
  'Slot Başarımları': ['Başlangıç Çarkı', 'Slotçu', 'Bağımlı', 'Kumarbaz I'], // 'Başlangıç Çarkı', 'Slotçu', 'Bağımlı', 'Kumarbaz I', 'Beleşçi', 'Cenabettin', 'Para Gözlü', 'Altın Çocuk', 'Çarkçı', 'Çark Ustası','Rulet Kralı','Kumarbaz II'
...
exports.run = async (client, message, args) => {
  ...
  const embed = new Discord.MessageEmbed()
    ...
    .setDescription(`> <:basarimlar:1188814034649235476>`);
  ...
  for (const category in groupedAchievements) {
    ...
    if (earnedAchievementsInCategory > 0) {
      ...
      embed.addField(`**${category} (${earnedAchievementsInCategory}/${totalAchievementsInCategory}):**`, '<:deny:1188812328070819900>');
    }
  }

  // Eğer hiç başarı alınmamışsa
  if (userAchievements.length === 0) {
    embed.addField('**Başarımlar:**', '<:deny:1188812328070819900>');
  }
