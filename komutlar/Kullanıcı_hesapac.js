...
exports.run = async (client, message, args, perms) => {
  ...
  if (!hesapdurumu) {
    ...
    if (client.ekoayarlar.rastgelepara == true) {
      ...
      const userInfoCommand = client.commands.get('pinfo'); // 'pinfo' komutunun adını düzenleyebilirsiniz
      userInfoCommand.run(client, message, [message.author.id]);
    } else {
      if (client.ekoayarlar.rastgelepara == false) {
        ...
        const userInfoCommand = client.commands.get('pinfo'); // 'pinfo' komutunun adını düzenleyebilirsiniz
        userInfoCommand.run(client, message, [message.author.id]);
        const gunlukInfoCommand = client.commands.get('daily'); // 'pinfo' komutunun adını düzenleyebilirsiniz
        gunlukInfoCommand.run(client, message, [message.author.id]);
      }
    }
  }
};
