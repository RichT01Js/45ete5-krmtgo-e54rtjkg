...
exports.run = async (client, message, args) => {
  ...
  setTimeout(() => {
    ...
    if (resultColorInfo.label === selectedColorInfo.label) {
      ...
      for (const achievement of achievements) {
        ...
        if (count < achievement.count && count + 1 >= achievement.count) {
          ...
          message.channel.send(`${user}, **${achievement.name}** başarımını kazandın! <:accepts:1188812293048369233>`);
        }
      }
    } else {
    ...
    spinningMessage.edit(`》${resultColorInfo.name}${resultColorInfo.name}${resultColorInfo.name}《\n━━━━━━━━━━━━━━━\n${user}, ${winAmount > 0 ? `**${formatCurrency(betAmount)}${client.ekoayarlar.parabirimi}** tutarında bahis yatırdın ve **${formatCurrency(winAmount)}${client.ekoayarlar.parabirimi}** kazandın. <a:slotwin:1188538218170286110>` : `**${formatCurrency(betAmount)}${client.ekoayarlar.parabirimi}** tutarında bahis yatırdın ve kaybettin. <a:slotlost:1188543753644933120>`}`);
