...
exports.run = async (client, message, args) => {
  ...
  if (winChance) {
    ...
    const winMessages = [
      ...
      `${user}, Bir adamın çantasını dızladın **${formattedAmountWon}** değerinde eşya sattın.`,
      `${user}, Yolda yürürken **${formattedAmountWon}** buldun. Şanslısın!`,
      `${user}, Bir kedi tarafından getirilen **${formattedAmountWon}**'yi kabul ettin.`,
      `${user}, Görünmez bir elf, cebine **${formattedAmountWon}** koydu. Garip ama gerçek!`,
      `${user}, Kaybolmuş bir hazine haritası buldun ve **${formattedAmountWon}** değerinde define buldun.`,
      `${user}, Güvercinler sürüsü senin etrafında dönüp dururken, yerde **${formattedAmountWon}** buldun.`,
      `${user}, Uyumadan önce yastığının altında **${formattedAmountWon}** gömülüydü. Şeker peri mi geldi acaba?`,
      `${user}, Yürüyüş yaparken rastgele birisi sana **${formattedAmountWon}** hediye etti.`,
      `${user}, Güneş batarken gökyüzünden **${formattedAmountWon}** yağdı. Altın yağmuru!`,
      `${user}, Rüyanda bir tane **${formattedAmountWon}** ağacı buldun ve uyandığında elindeydi.`,
    ];
